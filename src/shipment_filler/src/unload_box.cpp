...
int main(int argc, char** argv) {
    ...
    if (boxInspector.get_bad_part_Q1(current_part)) {
        ...
        float currentPartX = current_part.pose.pose.position.x;
        float currentPartY = current_part.pose.pose.position.y;
        float currentPartZ = current_part.pose.pose.position.z;
        float threshold = 0.005;
        int badPartIndex = -1;
        for(int i=0;i<nparts;i++){
        	if((abs(currentPartX - orphan_models_wrt_world[i].pose.position.x) < threshold) &&
        		(abs(currentPartY - orphan_models_wrt_world[i].pose.position.y) < threshold) &&
        		(abs(currentPartZ - orphan_models_wrt_world[i].pose.position.z) < threshold)){
        		badPartIndex = i;
        ...
        if(badPartIndex != -1){
        	...
        	ROS_INFO("couldn't find bad part");
        	return 0;
        }
